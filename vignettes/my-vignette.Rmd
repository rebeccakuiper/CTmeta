...
- Phi: The lagged effects matrix.  For each of the S = 3 studies, retrieve the q x q lagged effects matrix; in this specific example, there are three 2 x 2 lagged effects matrices. Stack the three matrices to form the lagged effects matrix (Phi) with dimensions S*q x q. This argument has an example matrix stored in the package:

```{r}
Phi <- myPhi
```

- SigmaVAR: The residual covariance matrix.  For each of the S = 3 studies, retrieve the q x q residual covariance matrix.  Stack the matrices to form a residual covariance matrix (SigmaVAR) with dimensions S*q x q. This argument has an example matrix stored in the package:

```{r}
SigmaVAR <- mySigmaVAR
```

- Gamma: The stacked stationary covariance matrix. For each of the S = 3 studies, generate the q x q stationary covariance matrix.  Then, stack the matrices to form the stationary covariance matrix (Gamma) with dimensions S*q x q. This argument has an example matrix stored in the package:
...
An example of a stacked matrix of size (3\*2) x 2 is presented below.
...
```{r}
# Option 2: Use the interactive web application:  (\url{https://www.uu.nl/staff/RMKuiper/Websites\%20\%2F\%20Shiny\%20apps}).
```
