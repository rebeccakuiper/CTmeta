...
Check_B <- function(B){
  ...
  if(is.null(dim(B))){
    if(!is.null(length(B))){
      print(paste0("The argument Drift is not a matrix. It should be a matrix of size q x q."))
      stop()
    }else{
    ...
    print(paste0("The lagged effects matrix Drift should be a q x q matrix."))
    stop()
  }else if(length(dim(B)) > 2){
    print(paste0("The lagged effects matrix Drift should be a q x q matrix."))
    stop()
    ...
    print(paste0("The lagged effects matrix Drift should be a square matrix of size q x q. Currently, it is of size ", dim(B)[1], " x ", dim(B)[2]))
    stop()
  }
