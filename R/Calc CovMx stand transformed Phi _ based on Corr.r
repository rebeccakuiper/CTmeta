...
#' @param waves Optional. Number of waves, that is, number of measurement occasions; which should be an integer of at least 2. By default, waves = 2.
#' @param corr_lagged The (lagged) correlation matrix of the q variables of size (q x waves) times (q x waves). The upper left q x q matrix is the correlation matrix between the q variables on the first time point; the upper right q x q matrix is the lagged correlation matrix between the q variables for the first and last time point (btw this block matrix and the other ones for non-subsequent time points will not be used to determine the order-1 relationships). By default, corr_lagged = NULL.
#' @param corr_YXYX Used if not NULL (default) & corr_lagged = NULL. Also the (lagged) correlation matrix of the q variables of size (q x waves) times (q x waves) but, in this matrix, the time points are ordered from high to low. Therefore, the upper left q x q matrix is the correlation matrix between the q variables on the last time point; the upper right q x q matrix is the lagged correlation matrix between the q variables for the last and first time point. By default, corr_YXYX = NULL.
...
TransPhi_Corr <- function(DeltaTStar, DeltaT = 1, N = NULL, waves = 2, corr_lagged = NULL, corr_YXYX = NULL, alpha = 0.05) {
  # TO DO Als waves > 2, dan evt ook extra deltaT... Dan wel code hieronder aanpassen, nl eerst transPhi -- ook handig voor volgende:
  # TO DO add option to obtain the wave-specific lagged relationships
  ...
  #
  # The length of the DeltaT's should be either 1 or (waves-1)
  if(length(DeltaTStar) != 1 & length(DeltaTStar) != (waves-1)){
    ErrorMessage <- (paste0("The argument 'DeltaTStar' should be a vector of length 1 or ('waves'-1). Currently, DeltaTStar = ", DeltaTStar))
    return(ErrorMessage)
    stop(ErrorMessage)
  }
  if(length(DeltaT) != 1 & length(DeltaTStar) != (waves-1)){
    ErrorMessage <- (paste0("The argument 'DeltaT' should be a vector of length 1 or ('waves'-1). Currently, DeltaT = ", DeltaT))
    return(ErrorMessage)
    stop(ErrorMessage)
  }
  #
  ...
  # Should be square matrix
    ...
    # If 3 waves, then correlations in corr_lagged are:
    # r(Y1_T0, Y1_T0), r(Y1_T0, Y2_T0), ...
    # r(Y2_T0, Y1_T0), ...
    # r(Y1_T1, Y1_T0), ...
    # r(Y2_T1, Y1_T0), ...
    # r(Y1_T2, Y1_T0), ...
  ...
  # Phi_w <- Phi_w[-1]
  # #
  # Phi <- apply(simplify2array(Phi_w), c(1,2), mean)
  #
  #
  # # Based on averaging correlation matrices
  ...
  if(length(ratioDeltaT) == 1 & waves > 2){
    ratioDeltaT <- c(NA, rep(ratioDeltaT, (waves-1)))
  ...
  for(w in 2:waves){
    ...
    if(ratioDeltaT[w] != 1){
      ...
      TransPhi_w[[w]] <- V %*% D^ratioDeltaT[w] %*% solve(V)
      #
  ...
  vecPhi <- as.vector(t(TransPhi))
    ...
    if(!is.null(N)){
      final <- list(DeltaTStar = DeltaTStar, DeltaT = DeltaT,
                    ...
                    #
                    standPhi_DeltaTStar = TransPhi, vecStandPhi_DeltaTStar = vecPhi,
